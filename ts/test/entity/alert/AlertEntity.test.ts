...
describe('AlertEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set MBTA_V__TEST_ALERT_ENTID JSON to run live')
      return
    }
    ...
    const alert_ref01_match_dt0: any = {}
    const alert_ref01_data_dt0 = await alert_ref01_ent.load(alert_ref01_match_dt0)
    assert(null != alert_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MBTA_V__TEST_ALERT_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'MBTA_V__TEST_ALERT_ENTID': idmap,
    'MBTA_V__TEST_LIVE': 'FALSE',
    'MBTA_V__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['MBTA_V__TEST_ALERT_ENTID']
