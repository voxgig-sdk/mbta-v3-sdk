...
[MCP](https://modelcontextprotocol.io) server exposing the MbtaV3 SDK as
two agent tools — `mbta-v3_list` and `mbta-v3_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mbta-v3-mcp)
make build

# 2. Provide credentials via the environment
export MBTA_V3_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mbta-v3 \
  -- /absolute/path/to/mbta-v3-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mbta-v3-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mbta-v3_list: first page of records
{ "entity": "alert" }
{ "entity": "alert", "query": { } }

// mbta-v3_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mbta-v3-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mbta-v3 \
     -- "$PWD"/dist/*/mbta-v3-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mbta-v3_list` and `mbta-v3_load` tools now appear
   in new sessions. Ask the agent to *"list alert using mbta-v3"*
   and it calls `mbta-v3_list` with `{"entity":"alert"}`.
...
```sh
export MBTA_V3_APIKEY=sk_live_xxx            # API key
export MBTA_V3_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `mbta-v3_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mbta-v3_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MBTA_V3_APIKEY` | API key sent with every request. |
| `MBTA_V3_BASE` | Optional override of the API base URL. |

### Entities

The 12 entities valid as the `entity` argument:

alert | facility | line | prediction | route | route_pattern | schedule | service | shape | stop | trip | vehicle

### Smoke test via HTTP (raw JSON-RPC)

```sh
./mbta-v3-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mbta-v3_load","arguments":{"entity":"alert","query":{"id":1}}}}'
```
