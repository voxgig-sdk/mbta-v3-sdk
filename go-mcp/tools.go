...
type Args struct {
	Entity string         `json:"entity" jsonschema:"alert | facility | line | prediction | route | route_pattern | schedule | service | shape | stop | trip | vehicle"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MbtaV3SDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "mbta-v3_list",
		Description: "List records from MbtaV3. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "mbta-v3_load",
		Description: "Load a single record from MbtaV3. " +
...
func runOp(client *sdk.MbtaV3SDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MbtaV3SDK, name string) (sdk.MbtaV3Entity, error) {
	switch strings.ToLower(name) {
	case "alert":
		return client.Alert(nil), nil
	case "facility":
		return client.Facility(nil), nil
	case "line":
	...
	case "route_pattern":
		return client.RoutePattern(nil), nil
	case "schedule":
		return client.Schedule(nil), nil
	case "service":
		return client.Service(nil), nil
	case "shape":
		return client.Shape(nil), nil
	case "stop":
		return client.Stop(nil), nil
	case "trip":
		return client.Trip(nil), nil
	case "vehicle":
