...
class FacilityEntity
  ...
  def make
    opts = @_entopts.dup
    FacilityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MbtaV3Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Facility, Hash] the current Facility data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MbtaV3Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Facility fields)
  def match_get
  ...
  #
  # @param reqmatch [FacilityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Facility.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Facility, Hash] the loaded Facility; raises MbtaV3Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MbtaV3Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
