...
class RoutePatternEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = route_pattern_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "route_pattern." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_ROUTE_PATTERN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    route_pattern_ref01_data = nil
    if route_pattern_ref01_data_raw.length > 0
...
def route_pattern_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "route_pattern", "RoutePatternTestData.json")
  ...
  entid_env_raw = ENV["MBTA_V3_TEST_ROUTE_PATTERN_ENTID"]
