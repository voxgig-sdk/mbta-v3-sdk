...
class PredictionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_PREDICTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def prediction_basic_setup(extra)
  ...
  entid_env_raw = ENV["MBTA_V3_TEST_PREDICTION_ENTID"]
