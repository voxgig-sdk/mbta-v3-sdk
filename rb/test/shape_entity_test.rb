...
class ShapeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = shape_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "shape." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MBTAV__TEST_SHAPE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    shape_ref01_data = nil
    if shape_ref01_data_raw.length > 0
...
def shape_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "shape", "ShapeTestData.json")
  ...
  entid_env_raw = ENV["MBTAV__TEST_SHAPE_ENTID"]
