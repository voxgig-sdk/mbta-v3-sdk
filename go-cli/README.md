...
boru-driven command-line client **and** interactive REPL for the MbtaV3
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mbta-v3-cli)
make build
...
./mbta-v3-cli load 1 alert            # {id:1} shorthand
./mbta-v3-cli load '{id:1}' alert       # explicit match map

# 5. Override the API base URL for a single call
MBTA_V3_BASE=https://api.example.com ./mbta-v3-cli load 1 alert

# 6. No arguments -> interactive REPL
./mbta-v3-cli
mbta-v3> load 1 alert
mbta-v3> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mbta-v3-cli
   ```
...
```sh
./mbta-v3-cli load 1 alert          # scalar shorthand for {id:1}
./mbta-v3-cli load '{id:1}' alert     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `mbta-v3>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mbta-v3-cli`. |
...
(`./mbta-v3-cli <expr>`) and interactively (the REPL), and expressions compose the
