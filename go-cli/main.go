// Command MbtaV3-cli is a CLI + REPL for the MbtaV3 SDK,
...
const prompt = "mbta-v3"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "alert facility line prediction route route_pattern schedule service shape stop trip vehicle"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MBTA_V3_APIKEY (api key), MBTA_V3_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MBTA_V3_APIKEY carries the API key and
	// MBTA_V3_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MBTA_V3_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MBTA_V3_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMbtaV3SDK(opts)
