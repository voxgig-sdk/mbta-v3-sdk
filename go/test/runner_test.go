...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MBTA_V3_TEST_LIVE") == "TRUE" ||
		os.Getenv("MBTA_V3_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MBTA_V3_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MbtaV3SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MbtaV3Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MbtaV3SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MbtaV3Error{Msg: msg, Code: code}
}
