...
func TestVehicleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MBTAV__TEST_VEHICLE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func vehicleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MBTAV__TEST_VEHICLE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"MBTAV__TEST_VEHICLE_ENTID": idmap,
		"MBTAV__TEST_LIVE":      "FALSE",
		"MBTAV__TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["MBTAV__TEST_VEHICLE_ENTID"])
