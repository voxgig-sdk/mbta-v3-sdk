...
func TestScheduleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MBTAV__TEST_SCHEDULE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func scheduleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MBTAV__TEST_SCHEDULE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MBTAV__TEST_SCHEDULE_ENTID"])
