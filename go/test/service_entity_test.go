...
func TestServiceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MBTAV__TEST_SERVICE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func serviceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MBTAV__TEST_SERVICE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MBTAV__TEST_SERVICE_ENTID"])
