...
func TestPredictionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_PREDICTION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		predictionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.prediction", setup.data)))
		var predictionRef01Data map[string]any
		if len(predictionRef01DataRaw) > 0 {
			predictionRef01Data = core.ToMapAny(predictionRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = predictionRef01Data

		// LOAD
		predictionRef01Ent := client.Prediction(nil)
		predictionRef01MatchDt0 := map[string]any{}
		predictionRef01DataDt0Loaded, err := predictionRef01Ent.Load(predictionRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if predictionRef01DataDt0Loaded == nil {
...
func predictionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"prediction01", "prediction02", "prediction03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("MBTA_V3_TEST_PREDICTION_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"MBTA_V3_TEST_PREDICTION_ENTID": idmap,
		"MBTA_V3_TEST_LIVE":      "FALSE",
		"MBTA_V3_TEST_EXPLAIN":   "FALSE",
		"MBTA_V3_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["MBTA_V3_TEST_PREDICTION_ENTID"])
