...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        MbtaV3SDK $client,
        MbtaV3Utility $utility
    ): MbtaV3Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new MbtaV3Context($ctxmap, null);
    ...
    private static function fixctx(MbtaV3Context $ctx, MbtaV3SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?MbtaV3Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MbtaV3Error($code, $msg);
    }
    ...
    {
        $hook_client = MbtaV3SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends MbtaV3BaseFeature {
            public $hook_fn = null;
            public function TestHook(MbtaV3Context $ctx): void
            {
    ...
    {
        $init_client = MbtaV3SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends MbtaV3BaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MbtaV3Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = MbtaV3SDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = MbtaV3SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof MbtaV3Error) ? $result->err->msg : (string)$result->err;
