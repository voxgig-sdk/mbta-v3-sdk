...
class PredictionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_PREDICTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $prediction_ref01_data = null;
        if (count($prediction_ref01_data_raw) > 0) {
...
function prediction_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["prediction01", "prediction02", "prediction03"] as $k) {
    ...
    $entid_env_raw = getenv("MBTA_V3_TEST_PREDICTION_ENTID");
