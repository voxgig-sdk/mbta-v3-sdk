...
class LineEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_LINE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function line_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["line01", "line02", "line03"] as $k) {
    ...
    $entid_env_raw = getenv("MBTA_V3_TEST_LINE_ENTID");
