...
class FtCtrl extends MbtaV3Control
{
...
class FtClient
{
    ...
    private MbtaV3Utility $utility;

    public function __construct(MbtaV3Utility $utility, array $options)
    {
    ...
    public function get_utility(): MbtaV3Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MbtaV3Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MbtaV3Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MbtaV3Config::make_feature($name);
    ...
    private static function buildUrl(MbtaV3Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MbtaV3Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MbtaV3Response($fetched);
        }
    ...
    private function fail(MbtaV3Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MbtaV3Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
