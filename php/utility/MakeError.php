...
class MbtaV3MakeError
{
    public static function call(?MbtaV3Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MbtaV3Context([], null);
        }
        $op = $ctx->op ?? new MbtaV3Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MbtaV3Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MbtaV3Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MbtaV3Error('', $msg, $ctx);
