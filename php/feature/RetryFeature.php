...
class MbtaV3RetryFeature extends MbtaV3BaseFeature
{
    ...
    public function init(MbtaV3Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MbtaV3Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MbtaV3Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MbtaV3Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
