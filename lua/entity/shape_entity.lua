...
local ShapeEntity = {}
ShapeEntity.__index = ShapeEntity


function ShapeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShapeEntity)
  self._name = "shape"
  self._client = client
...
function ShapeEntity:get_name()
  return self._name
end


function ShapeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShapeEntity.new(self._client, opts)
end
...
function ShapeEntity:mark_deleted()
  self._deleted = true
end


function ShapeEntity:deleted()
  return true == self._deleted
end


function ShapeEntity:data_set(args)
  if args ~= nil then
...
function ShapeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShapeEntity:match_set(args)
  if args ~= nil then
...
function ShapeEntity:match_get()
...
function ShapeEntity:stream(action, args, callopts)
...
function ShapeEntity:_run_op(ctx, post_done)
  local utility = self._utility
