...
local TripEntity = {}
TripEntity.__index = TripEntity


function TripEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TripEntity)
  self._name = "trip"
  self._client = client
...
function TripEntity:get_name()
  return self._name
end


function TripEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TripEntity.new(self._client, opts)
end
...
function TripEntity:mark_deleted()
  self._deleted = true
end


function TripEntity:deleted()
  return true == self._deleted
end


function TripEntity:data_set(args)
  if args ~= nil then
...
function TripEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TripEntity:match_set(args)
  if args ~= nil then
...
function TripEntity:stream(action, args, callopts)
...
function TripEntity:_run_op(ctx, post_done)
  local utility = self._utility
