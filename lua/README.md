...
The Lua SDK for the MbtaV3 API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Alert(nil):load({ id = "example_id" }, nil)
...
local result, err = client:MbtaV3(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── mbta-v3_sdk.lua    -- Main SDK module
...
The main module (`mbta-v3_sdk`) exports the SDK constructor
