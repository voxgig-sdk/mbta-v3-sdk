...
describe("AlertEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_ALERT_ENTID JSON to run live")
      return
    end
    ...
    local alert_ref01_data_dt0_loaded, err = alert_ref01_ent:load(alert_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(alert_ref01_data_dt0_loaded)
...
function alert_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MBTA_V3_TEST_ALERT_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["MBTA_V3_TEST_ALERT_ENTID"] = idmap,
