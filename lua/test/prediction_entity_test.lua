...
describe("PredictionEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MBTA_V3_TEST_PREDICTION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local prediction_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.prediction")))
    local prediction_ref01_data = nil
    if #prediction_ref01_data_raw > 0 then
      prediction_ref01_data = helpers.to_map(prediction_ref01_data_raw[1][2])
    end

    -- LOAD
    local prediction_ref01_ent = client:Prediction(nil)
    local prediction_ref01_match_dt0 = {}
    local prediction_ref01_data_dt0_loaded, err = prediction_ref01_ent:load(prediction_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(prediction_ref01_data_dt0_loaded)
...
function prediction_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MBTA_V3_TEST_PREDICTION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MBTA_V3_TEST_PREDICTION_ENTID"])
