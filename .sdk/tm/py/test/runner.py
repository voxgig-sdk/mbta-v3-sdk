...
class MbtaV3TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MbtaV3TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MbtaV3TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MbtaV3TestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = MbtaV3TestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MbtaV3TestRunner.getenv(key)
        ...
        explain = MbtaV3TestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MbtaV3TestRunner._test_control is not None:
            return MbtaV3TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MbtaV3TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MbtaV3TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MbtaV3TestRunner.load_test_control()
...
def load_env_local():
    MbtaV3TestRunner.load_env_local()


def env_override(m):
    return MbtaV3TestRunner.env_override(m)


def entity_data(v):
    return MbtaV3TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MbtaV3TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MbtaV3TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MbtaV3TestRunner.load_test_control()


def live_delay_ms():
    return MbtaV3TestRunner.live_delay_ms()
