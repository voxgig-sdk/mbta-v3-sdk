...
module MbtaV3Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = MbtaV3Context.new({}, nil)
    end
    op = ctx.op || MbtaV3Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || MbtaV3Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(MbtaV3Error) ? err.msg : err.to_s
    ...
    sdk_err = MbtaV3Error.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(MbtaV3Error)
