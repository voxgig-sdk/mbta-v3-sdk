# MbtaV3 SDK utility: make_request
...
module MbtaV3Utilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = MbtaV3Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MbtaV3Response.new(fetched)
    else
