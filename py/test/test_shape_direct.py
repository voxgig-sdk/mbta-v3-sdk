...
class TestShapeDirect:

    def test_should_direct_load_shape(self):
        setup = _shape_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-shape", "live" if setup["live"] else "unit")
        if _skip:
...
def _shape_direct_setup(mockres):
