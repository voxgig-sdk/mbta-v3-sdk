...
from mbtav3_sdk.mbtav3_types import (
    RoutePattern,
...
class RoutePatternEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RoutePatternEntity(self._client, opts)
    ...
    def data_get(self) -> RoutePattern:
    ...
    def match_get(self) -> RoutePattern:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RoutePattern:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RoutePattern().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
